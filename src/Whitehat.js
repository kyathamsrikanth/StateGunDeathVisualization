...
export default function Whitehat(props){
    ...
    const mapGroupSelection = useMemo(()=>{
        ...
        if(svg !== undefined & props.map !== undefined & props.data !== undefined){
            ...
            // Getting encode color based on deaths per 1000000
            const getEncodedFeature = d => Math.round(((d.count * 1000000) / d.population))
            ...
            function getStateInfo(sname){
                let name = cleanString(sname);
                let entry = stateData.filter(d=>d.state===name);
                if(entry === undefined | entry.length < 1){
                  return {gunDeaths:0,gunDeathsPer1K:0,statePoupulation:0}
                }
                let stateInfo= {
                    gunDeaths: entry[0].count,
                    gunDeathsPer1M : Math.round(((entry[0].count * 1000000) / entry[0].population)),
                    statePoupulation : entry[0].population,
                }
                return stateInfo;
            }
            ...
            mapGroup.selectAll('path').filter('.state')
                ...
                .on('mouseover',(_e,d)=>{
                    ...
                    let stateInfo = getStateInfo(sname);
                    let text = sname
                        + '</br>' + 'Gun Deaths per 1M: ' + stateInfo.gunDeathsPer1M
                        + '</br>' + 'State Population: ' + stateInfo.statePoupulation;
                    tTip.html(text);
                }).on('mousemove',(e)=>{
            ...
            mapGroup.selectAll('.city')
                ...
                .attr('opacity',.4)
                .on('mouseover', (_e, d) => {
                    tTip.html(d.city + '</br>' + 'City Count: ' + d.count);
                  }).on('mousemove', (e) => {
                    props.ToolTip.moveTTipEvent(tTip, e);
                  }).on('mouseout', (_e, _d) => {
                    props.ToolTip.hideTTip(tTip);
            ...
            function drawLegend(){
                ...
                const legendTitle = {
                    'x': legendX - barWidth - 6,
                    'y': bounds.y + 10 * barHeight + 173,
                    'text': 'Gun Deaths per 1 Million' 
                }
                ...
                svg.selectAll('.legendText')
                    ...
                    .attr('font-size',(_d,i) => i == 0? 1.2*fontHeight:fontHeight)
                    .text(d=>d.text);
            }
